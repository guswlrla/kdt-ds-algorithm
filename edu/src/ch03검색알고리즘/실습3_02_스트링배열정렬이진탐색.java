...
* 함수(메소드) 전체를 작성하는 훈련 
*/
public class 실습3_02_스트링배열정렬이진탐색 {
	...
	private static int binarySearch(String[] data, String key) {
		...
		do {
			...
			else if(data[pc].compareTo(key) < 0) { // 검색범위를 뒤쪽 절반으로 좁힘
				pl = pc + 1;
			}
	...
	public static void main(String[] args) {
		...
		showData("정렬전 ", data);
		sortData(data); // 올림차순으로 정렬 교재 p.211-212 단순 선택 정렬 알고리즘으로 구현
		showData("정렬후 ", data);

		String key = "포도";
		int resultIndex = linearSearch(data, key); // 교재 p.100 seqSearch() 함수로 구현
		System.out.println("\nlinearSearch(포도): key = " + key + ", result 색인 = " + resultIndex);

		key = "배";
		resultIndex = binarySearch(data, key); // 교재 p.109 binSearch() 함수로 구현
		 ...
		 */
		key = "산딸기";
		resultIndex = Arrays.binarySearch(data, key); // 교재 p.120 API 참조
		// 교재 p.116 표 3-3: static int binarySearch(Object[] a, Object key)가 사용됨 - 단 배열 a는 Comparable을 구현한 객체들로 정렬되어 있어야 함
