 ...
 * Rook/가로,세로 이동/다른 기물을 넘을 수 없다, bishop/대각선, knight/1-2칸 이동/다른 기물을 넘을 수 있다.
 * pawn/처음 이동은 2칸까지 가능, 그 후 한칸만 가능
 * 잡을 때는 대각선 가능 체스판 최대 배치 문제 : king/16
...
public class NQueen {
	...
	private void solve() {
		...
		// - 퀸을 배치했으면, 다른 형태의 배치 방법이 있는 지 확인
		// - 퀸을 배치했는데, 다른 형태의 배치 방법이 없으면 => 백트래킹
		while(true) {
			boolean placed = false; // 위치했어?
			// 현재 행에서 퀸을 배치할 수 있는 "열" 찾기
			while(col < BOARD_SIZE) { // 보드사이즈 보다 작으면
				if(isSafe(row, col)) { // 안전해?
					board[row][col] = 1; // 퀸 배치
					queens.push(new Position(row, col)); // 스택에 0,0 넣음
					placed = true;
					break;
				}
				col++;
	...
	public static void main(String[] args) {
		...
		solver.solve();
		System.err.println("모든 해 : " + solver.getSolutionCount());
	}
}
