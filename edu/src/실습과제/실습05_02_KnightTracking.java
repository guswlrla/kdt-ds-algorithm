...
public class 실습05_02_KnightTracking {
	 ...
	 * 
	 * 문제 설명 체스판은 보통 8x8 크기이지만, 이 문제는 임의의 N x N 체스판에서 해결할 수 있습니다.
	...
	static final int N = 8; // 체스판 크기
	
	
	private static int[][] board = new int[N][N]; // 체스판 배열
	...
	// 경계조건 및 가장 중요한 valid 함수
	private static boolean isSafe(int x, int y) {
		return (x >= 0 && x < N && y >= 0 && y < N && board[x][y] == -1); // 체스판 위에 있고 한번도 안 지나갔느냐?
	}
	...
	private static ArrayList<Move> getOrderMoves(int x, int y) {
		...
		for(int i = 0; i < 8; i++) {
			...
			// 2. 접근성이 낮은 순대로 정렬(위슨드로프의 휴리스틱 사용)
			if(isSafe(nextX, nextY)) {
				int accessibility = countAccessibility(nextX, nextY); // 카운트 해야함
